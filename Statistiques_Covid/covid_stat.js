...
function getCovidStats() {
    fetch('https://coronavirus-tracker-api.herokuapp.com/v2/locations/248')
        ...
        document.getElementById('update').innerHTML = "update :" + update.substr(0, 10);
        document.getElementById('cases').innerHTML = "cases :" + confirmedCases.toLocaleString('en');
        document.getElementById('deaths').innerHTML = "death :" + deaths.toLocaleString('en');
        document.getElementById('percent').innerHTML = "percent :" + ((Number(deaths) / Number(confirmedCases)) * 100).toLocaleString("en", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + "%";
    })["catch"](function () {
        console.log("error");
    });
